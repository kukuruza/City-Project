...
max_votingMap = max(max(votingMap(1:interval:round(imageH*uppPercent),borderWidth+1:interval:imageW-borderWidth)));
min_votingMap = min(min(votingMap(1:interval:round(imageH*uppPercent),borderWidth+1:interval:imageW-borderWidth)));
votingMap1 = (votingMap(1:interval:round(imageH*uppPercent),borderWidth+1:interval:imageW-borderWidth)-min_votingMap)*255/(max_votingMap-min_votingMap);
votingMap(1:interval:round(imageH*uppPercent),borderWidth+1:interval:imageW-borderWidth) = votingMap1;

tmpVotingMap = votingMap;
tmpVotingMap(round(0.9*imageH):imageH,:)=0;
votingMap = tmpVotingMap;


www=5;
sum_votingMap = zeros(imageH,imageW);
for iii=1+www:imageH-www
    for jjj=1+www:imageW-www
...
max_votingMap = max(max(tmpVotingMap(1:1:round(imageH*uppPercent),borderWidth+1:1:imageW-borderWidth)));
[r,c] = find(tmpVotingMap(1:1:round(imageH*uppPercent),borderWidth+1:1:imageW-borderWidth)==max_votingMap);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
%     thr = adaptiveThreholding1(votingMap(1:round(uppPercent*imageH),:),1);
...
%     if tta>=(0.5*imageW)||(zeroLength/spikeLength>=0.75)  %%deviate to left or right
%         [clusterLabels, numOfAllClusters] = bwlabel(segImg);
%            if numOfAllClusters>=1  %%% always larger than or equal to 1
%                allClustersData = regionprops(clusterLabels,'basic');
%                clusterArea = 0;
...
%        
           ...
           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           votingArea = zeros(imageH,imageW);  %%%%%%%%%%%%%%%%%%%%%%%%%%%%% voting area
            ...
            for i=3:33
                    ...
                    if tmpAngle1~=90
                        ...
                        if tmpAngle1<90
                            yJoint = round(tan(tmpAngle)*(imageW-c)+r);
                            if yJoint<=imageH
                                tempDist1(i-2) = sqrt((imageW-c)*(imageW-c) + (yJoint-r)*(yJoint-r));
                            else
                                xJoint = (imageH-1 - r)/(tan(tmpAngle)) + c;
                                tempDist1(i-2) = sqrt((xJoint-c)*(xJoint-c) + (imageH-r)*(imageH-r));
                            end
                        else
            ...
            tempDist1_binary = double(tempDist1>=imageH*0.5);
            ...
            for i=1:length(largestCluster)
                ...
                if angleCC<60   
                    ...
                    tmp_y = round(tan(angleCCRad)*(imageW-c)+r);
                    tmp_y1 = round(tan(tmpAngle1Rad)*(imageW-c)+r);
                    tmp_y2 = round(tan(tmpAngle2Rad)*(imageW-c)+r);
                    if (tmp_y1<=imageH)&&(tmp_y2<=imageH)
                        for x=c:imageW
                            y=round(tan(angleCCRad)*(x-c)+r);
                            y1=round(tan(tmpAngle1Rad)*(x-c)+r);
                            y2=round(tan(tmpAngle2Rad)*(x-c)+r);
                            if (y>=r)&&(y<=imageH)&&(y1>=r)&&(y1<=imageH)&&(y2>=r)&&(y2<=imageH)
                                emptyImg(y1:y,x)=1;
                                emptyImg(y:y2,x)=2;
                            end
                        end
                    elseif (tmp_y1<=imageH)&&(tmp_y2>imageH)
                        tmp_x2 = round((imageH-r)/tan(tmpAngle2Rad)+c);
                        if tmp_x2>imageW 
                            tmp_x2=imageW; 
                        end
                        if tmp_y<=imageH
                            for x=c:imageW
                                y=round(tan(angleCCRad)*(x-c)+r);
                                y1=round(tan(tmpAngle1Rad)*(x-c)+r);
                                if (y>=r)&&(y<=imageH)&&(y1>=r)&&(y1<=imageH)
                                    emptyImg(y1:y,x)=1;
                                end
                            end
                            for x=c:tmp_x2
                                y=round(tan(angleCCRad)*(x-c)+r);
                                y2=round(tan(tmpAngle2Rad)*(x-c)+r);
                                if (y>=r)&&(y<=imageH)&&(y2>=r)&&(y2<=imageH)
                                    emptyImg(y:y2,x)=2;
                                end
                            end
                            for x=tmp_x2:imageW
                                y=round(tan(angleCCRad)*(x-c)+r);
                                if (y>=r)&&(y<=imageH)
                                    emptyImg(y:imageH,x)=2;
                                end
                            end
                        else
                            tmp_x = round((imageH-r)/tan(angleCCRad)+c);
                            if tmp_x>imageW 
                                tmp_x=imageW; 
                            end
                            for x=c:tmp_x
                    ...
                    elseif (tmp_y1>imageH)&&(tmp_y2>imageH)
                        tmp_x1 = round((imageH-r)/tan(tmpAngle1Rad)+c);
                        tmp_x2 = round((imageH-r)/tan(tmpAngle2Rad)+c);
                        tmp_x = round((imageH-r)/tan(angleCCRad)+c);
                        if tmp_x>imageW 
                            tmp_x=imageW; 
...
%             angleTheta1 = tmpIndx*5;
            ...
            if para(1)>=0
                ...
                jointOfImh = (imageH-r)/(para(1)) + c;
                jointOfImhCopy = min(jointOfImh, imageW);
                for tempFixedVp_X= c-2:c+2; %2:imageW-3 %%c-1:c+1 %%3:5:endPoint
                    ...
                    if (tempFixedVp_Y>1)&&(tempFixedVp_Y<imageH)
                        ...
                        for iii=1:numOfAngleChoices
                            ...
                            if (tempCurrentAngle~=90)%%&&(abs(tempCurrentAngle-angleTheta1)>=35)
                                if tempCurrentAngle<90
                                    yJoint = round(tanVal(iii)*(imageW-tempFixedVp_X)+tempFixedVp_Y);
                                    if yJoint<=imageH
                                        tempDist1 = sqrt((imageW-tempFixedVp_X)*(imageW-tempFixedVp_X) + (yJoint-tempFixedVp_Y)*(yJoint-tempFixedVp_Y));
                                    else
                                        xJoint = (imageH-1 - tempFixedVp_Y)/(tanVal(iii)) + tempFixedVp_X;
                                        tempDist1 = sqrt((xJoint-tempFixedVp_X)*(xJoint-tempFixedVp_X) + (imageH-tempFixedVp_Y)*(imageH-tempFixedVp_Y));
                                    end
                                else
    ...
    %                             tempCounterRatio(iii) = tempCounter0/tempCounter;
                            elseif (tempCurrentAngle==90)%%&&(abs(tempCurrentAngle-angleTheta1)>=35)
                                tempCounter = imageH-tempFixedVp_Y;
                                tempDist1 = tempCounter;
                                for jjj=tempFixedVp_Y:imageH
    ...
    %                             tempCounterRatio(iii) = tempCounter0/tempCounter;
                            ...
                            tempDist = sqrt(jointOfImh*jointOfImh + (imageH-tempFixedVp_Y)*(imageH-tempFixedVp_Y));
                            if (tempCounterRatio>0.02)&&(abs(tempCurrentAngle-angleTheta1)>=20)&&(tempDist>0.35*imageH)&&(tempDist1>0.35*imageH)%%(tempCounterRatio>sumOftempCounterRatio_max)&&
...
%                         end
                ...
                for tempFixedVp_X=c+2:-1:c-2 %%c-1:c+1 %%imageW-3:-5:endPoint
                    %%%%%% find the second dominant edge
                    %%%%%% the angle range of the second dominant edge
                    jointOfImh = jointOfImhCopy;
                    tempFixedVp_Y = round(para(1)*(tempFixedVp_X-c)+r);
...
%             selected_tempCounterRatio
            ...
            if sum(selectedVp)>0
                ...
                newC = round(0.5*imageW);
                newR = round(0.5*imageH);
          ...
          doim = colorImg;
            ...
            for iii=1:8
                ...
                if (para(1)>=0)&&(tanVal(selected_constructedLine(iii))>=0)
                    ...
                    for x=newC:imageW
                        y1 = min(round(para(1)*(x-newC)+newR),imageH);
                        y2 = min(round(tanVal(selected_constructedLine(iii))*(x-newC)+newR),imageH); %%round(tanVal(iii)*(x-newC)+newR);
                        if (y2>=newR)&&(y2<=imageH-1)
                            doim(y2-1:y2+1,x,1) = 0;
                            doim(y2-1:y2+1,x,2) = 255;
                            doim(y2-1:y2+1,x,3) = 0;
                        end
                        if (y1>=newR)&&(y1<=imageH)&&(y2>=newR)&&(y2<=imageH)
                            doim(y2-1:y2+1,x,1) = 0;
                            doim(y2-1:y2+1,x,2) = 255;
                            doim(y2-1:y2+1,x,3) = 0;
                            if y1>=y2
                ...
                elseif (para(1)>=0)&&(tanVal(selected_constructedLine(iii))<=0)
                    for x=1:newC
                        y2 = min(round(tanVal(selected_constructedLine(iii))*(x-newC)+newR),imageH); 
                        if (y2>=newR)&&(y2<=imageH-1)
                            y2a = max(y2-10,2);
                            doim(y2-1:y2+1,x,1) = 0;
                            doim(y2-1:y2+1,x,2) = 255;
                            doim(y2-1:y2+1,x,3) = 0;
                            votingArea(y2a:imageH,x)=1;
                        end
                    end
                    for x=newC:imageW
                        y1 = min(round(para(1)*(x-newC)+newR),imageH-1);
                        if (y1>=newR)&&(y1<=imageH-1)
                            y1 = max(y1-10,1);
                            votingArea(y1:imageH,x)=1;
                        end
                    end
                elseif (para(1)<=0)&&(tanVal(selected_constructedLine(iii))<=0)
                    for x=1:newC
            ...
            %%%%%% rough road area 
                        ...
                        doim = colorImg;
                        doim1 = colorImg;
                        roadBinaryImage = zeros(imageH, imageW);
                                ...
                                if angleTheta1>90
                                    if mean_forClustering==90
                                        for x=1:newC
                                            ...
                                            if (y>=newR)&&(y<=imageH)
                                                doim1(y:imageH,x,1) = 255;
                                                roadBinaryImage(y:imageH,x) = 255;
                                            end
                                        end
                                    else
                                        for x=1:newC
