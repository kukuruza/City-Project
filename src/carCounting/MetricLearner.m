...
classdef MetricLearner < handle
    ...
    methods
        ...
        function [newCarNumber Match] = processFrame (ML, image, carcell, geometryObj)
            ...
            ProbCol = zeros(length(carapps), length(ML.seenCarapps));
            ProbHOG = zeros(length(carapps), length(ML.seenCarapps));
            ProbWeighted = zeros(length(carapps), length(ML.seenCarapps));
            
 
            for i = 1 : length(carapps)
                carapp = carapps{i};      % all the cars in new frame
                for j = 1 : length(ML.seenCarapps)
                    seenCarapp = ML.seenCarapps{j};   % all the cars in former frame
                   
                    ProbGeo(i,j) = geometryObj.getMutualProb(seenCarapp, carapp, 1);
                    probMap = geometryObj.generateProbMap(seenCarapp, 1, image);
                    imshow(probMap)
                    pause()
                    fprintf('Prob : %f\n', ProbGeo(i,j));
                    [ProbCol(i,j), ProbHOG(i,j)] = ML.AppProb(carapp, seenCarapp);
