function score = detApplyDetectorContext(Img, cimages, dict, paramClassifier, scaleSampling, maxImageSize, objsize)
%
...
while min(size(img))>32
    ...
    cFeatures = contextFeatures(cimages, objsize);
    ...
    if max(size(img)) <= maxImageSize 
        for j = 1:NweakClassifiers
            ...
            if f <= length(dict)
                ...
                feature = cFeatures(:, :, f-length(dict));
            end
    ...
    if scaleSampling < 1
        ...
        cimages = imresize(cimages, scaleSampling, 'bilinear');             
    else
        score = Score;
        break
    end
